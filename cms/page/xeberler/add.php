<?PHP
	...
	include 'upload_photos.php';
		...
		$ddd=mysqli_query($connection,'select max(u_id) as uid from site_xeberler');
		$bbb=MYSQLI_FETCH_ARRAY($ddd);
		$u_id=$bbb['uid'];
		$u_id++;
		
		$zzz=mysqli_query($connection,'select max(ordering) as uid from site_xeberler');
		...
		for($i=1; $i<= $hidsay; $i++ ){
			...
			@$insert = MYSQLI_QUERY($connection,"INSERT INTO `site_xeberler` (
				`name`, 
				`text`, 
				`text2`,
				`l_id`, 
