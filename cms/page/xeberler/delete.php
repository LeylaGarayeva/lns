<?PHP
	...
	elseif(@$delete){
		...
		$product22=MYSQLI_QUERY($connection,"SELECT * from `site_xeberler` where l_id='1' and u_id='".$id."'");
		...
		for($i=1; $i<= $hidsay; $i++ ){
			$delete = MYSQLI_QUERY($connection,"DELETE FROM site_xeberler WHERE l_id='".$i."' AND u_id='".$id."'"); 
		}
		if($delete){
