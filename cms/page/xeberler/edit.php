...
$tip=0;
if(!empty($avatar)){
$product22=MYSQLI_QUERY($connection,"SELECT * from `site_xeberler` where l_id='1' and u_id='".$idx."'");		$product=MYSQLI_FETCH_ARRAY($product22);		if(file_exists('../products/'.$product['photo'].'')){		unlink('../products/'.$product['photo'].'');		}		else{}
		...
		if(@isset($_POST['checkbox'])){
			...
			$text= $_POST['text'.$i];
			if(empty($avatar)){
			$update = MYSQLI_QUERY($connection,"UPDATE site_xeberler SET  `name` = '$name',`checkbox` = '$checkbox', `text` = '$text', `text2` = '$text2', `status` = '$active', `url_tag` = '$url_tag', `title` = '$title', `keyword` = '$keyword', `description` = '$description' WHERE u_id='".$idx."' AND l_id='".$i."'");
			}
			else{
			
			$update = MYSQLI_QUERY($connection,"UPDATE site_xeberler SET  `name` = '$name',`checkbox` = '$checkbox', `text` = '$text', `text2` = '$text2', `status` = '$active', `url_tag` = '$url_tag', `title` = '$title', `keyword` = '$keyword', `description` = '$description',`photo` = '$avatar' WHERE u_id='".$idx."' AND l_id='".$i."'");
			}
		...
		<form action="" method="POST" enctype="multipart/form-data">
			<table cellpadding="0" cellspacing="0" border="0" align="center">
				...
				<tr>
					<td>
					<?PHP
					$s_redakte1=mysqli_query($connection,'SELECT * FROM site_xeberler WHERE u_id='.$idx.' AND l_id=1');
					...
					<div>
						<form action="">
							...
							<?php $dm=MYSQLI_QUERY($connection,"SELECT * FROM site_xeberler WHERE u_id='".$idx."'");
				...
				<table cellpadding="0" cellspacing="0" border="0">
					<tr>
						<td><b><?php echo $dil['Şəkli seç'][$lng]; ?></b><br><input type="file" name="dosya" /></td>
					</tr>
					 ...
					 <link rel="stylesheet" href="js/jquery-ui.css">
						...
						<div id="tabs">
								...
								<?PHP
									...
									while($nl = MYSQLI_FETCH_ARRAY($sl)){
									$dm=MYSQLI_QUERY($connection,"SELECT * FROM site_xeberler WHERE u_id='".$idx."' AND l_id='".$nl['id']."'");
									$dmm=MYSQLI_FETCH_ASSOC($dm);
