...
$tip=0;
if(!empty($avatar)){
$delphoto = MYSQLI_QUERY($connection,"SELECT image_url FROM kateqoriyalar WHERE u_id='".$u_id."'");
...
unlink('../products/'.$delphoto22['image_url']);
			...
			if(empty($avatar)){
			$update1 = MYSQLI_QUERY($connection,"UPDATE kateqoriyalar SET  `name` = '$name_az', `link` = '$link',sub_id='$tip'  WHERE kat_id='".$kat_id."' and l_id=1");
			$update2 = MYSQLI_QUERY($connection,"UPDATE kateqoriyalar SET  `name` = '$name_ru', `link` = '$link'  WHERE kat_id='".$kat_id."' and l_id=2");
			$update3 = MYSQLI_QUERY($connection,"UPDATE kateqoriyalar SET  `name` = '$name_en',`link` = '$link'  WHERE kat_id='".$kat_id."' and l_id=3");
			}
			else{
			
			$update1 = MYSQLI_QUERY($connection,"UPDATE kateqoriyalar SET  `name` = '$name_az', `link` = '$link', `image_url` = '$avatar',sub_id='$tip' WHERE kat_id='".$kat_id."'  and l_id=1");
			$update2 = MYSQLI_QUERY($connection,"UPDATE kateqoriyalar SET  `name` = '$name_ru', `link` = '$link', `image_url` = '$avatar',sub_id='$tip' WHERE kat_id='".$kat_id."'  and l_id=2");
			$update3 = MYSQLI_QUERY($connection,"UPDATE kateqoriyalar SET  `name` = '$name_en', `link` = '$link', `image_url` = '$avatar',sub_id='$tip' WHERE kat_id='".$kat_id."'  and l_id=3");
			}
		...
		if(isset($update1) && isset($update2) & isset($update3)){
			echo "<br><br><center><b><font size='4' color='red'> ".$dil['Redakte olundu'][$lng]."! </font></b></center></br><br>
				<script>
				function redi(){
				document.location='?menu=kateqoriyalar&tip=1&pos=1&p_id=1'
				}
		...
		<form action="" method="POST" enctype="multipart/form-data">
			<table cellpadding="0" cellspacing="0" border="0" align="center">
				...
				<tr>
					<td>
					<?PHP
					$s_redakte1=mysqli_query($connection,'SELECT * FROM kateqoriyalar WHERE kat_id='.$kat_id.' ');
					 ...
					 <link rel="stylesheet" href="js/jquery-ui.css">
						...
						<div id="tabs">
								...
								<?PHP
									...
									$dm=MYSQLI_QUERY($connection,"SELECT * FROM kateqoriyalar WHERE l_id='".$nl['id']."' and kat_id='".$kat_id."'  ");
