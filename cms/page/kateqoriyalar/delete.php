<?PHP
	...
	elseif(@$_POST['delete']){
		$select = MYSQLI_QUERY($connection,"SELECT * FROM kateqoriyalar WHERE kat_id='".$kat_id."'");
		$n = MYSQLI_FETCH_ASSOC($select);
		@unlink($_SERVER['DOCUMENT_ROOT'].'/Bakujaluzi/products/'.$n['photo']);
		$hidsay = $_POST['hidsay'];
		for($i=1; $i<= $hidsay; $i++ ){
			$delete = MYSQLI_QUERY($connection,"DELETE FROM kateqoriyalar WHERE kat_id='".$kat_id."'"); 
		}
		if($delete){
