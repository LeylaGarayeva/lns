...
function return_all_cats($sub_id=0){
	...
	if(@$_POST['send']){
		...
		function getMax($connection) {
			...
			$q="select kat_id from kateqoriyalar order by kat_id desc limit 1";
		...
		function getMax_Order($connection) {
			$q="select max(ordering) from kateqoriyalar";
			$res = mysqli_query($connection,$q);
			$b=mysqli_fetch_array($res);
			return $b[0];
		}
			...
			$getmax_order = getMax_Order($connection)+1;
				...
				$insert_az = MYSQLI_QUERY($connection,"INSERT INTO kateqoriyalar(kat_id, name, ordering, sub_id, l_id) values('".$getmax."', '".$name_az."', '".$getmax_order."', '".$tip."', '1')") or die('error1 '.mysqli_error($connection));
				
				$insert_az = MYSQLI_QUERY($connection,"INSERT INTO kateqoriyalar(kat_id, name, ordering, sub_id, l_id) values('".$getmax."', '".$name_az."', '".$getmax_order."', '".$tip."', '2')") or die('error1 '.mysqli_error($connection));
				
				$insert_az = MYSQLI_QUERY($connection,"INSERT INTO kateqoriyalar (kat_id, name, ordering, sub_id, l_id) values('".$getmax."', '".$name_az."', '".$getmax_order."', '".$tip."', '3')") or die('error1 '.mysqli_error($connection));
			
			if($insert_az){
				echo "<br><br><center><b><font size='4' color='red'> " .$dil['Əlavə olundu'][$lng]."! </font></b></center></br><br>
				<script>
				function redi(){
				document.location='?menu=kateqoriyalar'
				}
