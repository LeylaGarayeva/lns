<?PHP
	...
	if(isset($save)){
			...
			foreach($array as $k=>$vpppal) {
				foreach($vpppal as $ooo){
					$supdate = MYSQLI_QUERY($connection,"UPDATE olkeler SET ordering='".$ooo."' WHERE kat_id ='".$k."' ");
				}
			}
			if($supdate){
				...
				<script>
				function redi(){
				document.location='?menu=olkeler&catid=".$catid."'
				}
	...
	if(isset($delete)){
			...
			{
				$s_del = MYSQLI_QUERY($connection,"DELETE FROM olkeler WHERE kat_id = '".$chek[$n]."'");
			}
			if($s_del){
	...
	if(isset($aktiv)){
			...
			for($n=0; $n<count($chek); $n++)
			{
				$supdate = MYSQLI_QUERY($connection,"UPDATE olkeler SET s_id='0' WHERE u_id = '".$chek[$n]."'");
			}
			if($supdate){
	...
	if(isset($passiv)){
			...
			for($n=0; $n<count($chek); $n++)
			{ 
				$supdate = MYSQLI_QUERY($connection,"UPDATE olkeler SET s_id='1' WHERE u_id = '".$chek[$n]."'");
			}
			if($supdate){
	...
	<table>
		...
		<tbody>
			<?PHP
				...
				$s_m = MYSQLI_QUERY($connection, "SELECT * FROM olkeler where l_id='".$diller[$lng]."' ORDER BY ordering ASC");
				while($n_m = MYSQLI_FETCH_ASSOC($s_m)){
					...
					<tr>
						...
						<td>
								<!-- Icons -->
								<a href="?menu=olkeler&mod=edit&kat_id=<?PHP echo $n_m['kat_id'];?>" title="<?php echo $dil['Düzəliş et'][$lng]; ?>">
								<img src="images/pencil.png" alt="Düzəliş et" />
								</a>&nbsp;
								<a href="?menu=olkeler&mod=delete&kat_id=<?PHP echo $n_m['kat_id'];?>" title="<?php echo $dil['Sil'][$lng]; ?>">
