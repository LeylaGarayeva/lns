...
$tip=0;
if(!empty($avatar)){
$delphoto = MYSQLI_QUERY($connection,"SELECT photo FROM jurnal WHERE u_id='".$idx."' AND l_id='1'");
...
unlink('../products/'.$delphoto22['photo']);
		...
		for($i=1; $i<= $hidsay; $i++ ){
			...
			if(empty($avatar)){ 
			$update = MYSQLI_QUERY($connection,"UPDATE jurnal SET  `name` = '$name',`note` = '$note',`text` = '$text', `s_id` = '$active'  WHERE u_id='".$idx."' AND l_id='".$i."'");
			}
			else{
			
			$update = MYSQLI_QUERY($connection,"UPDATE jurnal SET  `name` = '$name',`note` = '$note',`text` = '$text',`s_id` = '$active', `photo` = '$avatar'  WHERE u_id='".$idx."' AND l_id='".$i."'");
			}
		...
		<form action="" method="POST" enctype="multipart/form-data">
			<table cellpadding="0" cellspacing="0" border="0" align="center">
				...
				<tr>
					<td>
					<?php $dm=MYSQLI_QUERY($connection,"SELECT * FROM jurnal WHERE u_id='".$idx."'");
				...
				<tr>
						...
						<div id="tabs">
								...
								<?PHP
									...
									while($nl = MYSQLI_FETCH_ARRAY($sl)){
									$dm=MYSQLI_QUERY($connection,"SELECT * FROM jurnal WHERE u_id='".$idx."' AND l_id='".$nl['id']."'");
									$dmm=MYSQLI_FETCH_ASSOC($dm);
								...
								<table>
									...
									<tr>
										<td>Note: </td>
										<td><input name="note<?PHP echo $nl['id']; ?>" style="width:500px;border-radius:5px;" value="<?PHP echo $dmm['note']; ?>"></td>
									</tr>
									<tr>
