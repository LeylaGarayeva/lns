<?PHP
	...
	include 'upload_photos.php';
		...
		$ddd=mysqli_query($connection,'select max(u_id) as uid from jurnal');
		$bbb=MYSQLI_FETCH_ARRAY($ddd);
		$u_id=$bbb['uid'];
		$u_id++;
		
		$zzz=mysqli_query($connection,'select max(ordering) as uid from jurnal');
		...
		for($i=1; $i<= $hidsay; $i++ ){
			...
			@$insert = MYSQLI_QUERY($connection,"INSERT INTO `jurnal` (
				`name`,
				`note`,
				`text`, 
				`l_id`, 
				    ...
				    <td>
						<div id="tabs">
								...
								<table>
									...
									<tr>
										<td>Note: </td>
										<td><input name="note<?PHP echo $nl['id']; ?>" style="width:500px;border-radius:5px;"></td>
									</tr>
									<tr>
