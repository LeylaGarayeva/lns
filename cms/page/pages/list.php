...
<table>
	...
	<tbody>
		...
		function list_menu($sub, $u_id, $n, $from, $page_count,$margin,$conn, $lng, $dil){
					...
					$sorgu = "SELECT * FROM site_menu WHERE sub_id = '".$sub."' AND tip IN(1,2,3) AND l_id=1 ORDER BY ordering ASC"
					or die($sorgu);
					$s_m = mysqli_query($conn,$sorgu);

					// $s_m = mysqli_query($conn,"SELECT * FROM menu WHERE sub_id = '".$sub."' AND tip = '".$tip."' AND l_id=1 ORDER BY ordering ASC");
					$says_m=mysqli_num_rows($s_m);
					if($says_m != 0){
						while($n_m = MYSQLI_FETCH_ASSOC($s_m)){
							?>
							...
							<tr>
								...
								<td>
									<!-- Icons -->
									 <a href="?menu=pages&mod=edit&tip=<?PHP echo $tip; ?>&id=<?PHP echo $n_m['u_id'];?>" title="<?php echo $dil['Düzəliş et'][$lng]; ?>">
										<img src="images/pencil.png" alt="Düzəliş et" />
									 </a>
									 <a href="?menu=pages&mod=delete&tip=<?PHP echo $tip; ?>&id=<?PHP echo $n_m['u_id'];?>" title="<?php echo $dil['Sil'][$lng]; ?>">
