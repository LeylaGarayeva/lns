<?PHP
	...
	if(@$send){
		...
		if(!empty($avatar)){
		$delphoto = MYSQLI_QUERY($connection,"SELECT photo FROM site_menu WHERE u_id='".$id."' AND l_id='1'");
		...
		for($i=1; $i<= $hidsay; $i++ ){
			...
			if(empty($avatar)){
			$update = MYSQLI_QUERY($connection,"UPDATE site_menu SET sub_id='$sub', name='$name', text='$text', text2='$text2', status='$active',  url_tag='$url_tag', title='$title',keyword='$keyword', description='$description', link='$link' WHERE u_id='$id' AND l_id='$i'");
			}
			else
			{
				$update = MYSQLI_QUERY($connection,"UPDATE site_menu SET sub_id='$sub', name='$name', text='$text', text2='$text2', status='$active', url_tag='$url_tag', title='$title',keyword='$keyword', description='$description', `photo` = '$avatar', link='$link' WHERE u_id='$id' AND l_id='$i'");
			}
		}
		if($update){
		...
		<form action="" method="POST" enctype="multipart/form-data">
			<table cellpadding="0" cellspacing="0" border="0" align="center">
				...
				<tr>
					<td>
					<?PHP
					$s_redakte1=mysqli_query($connection,'SELECT * FROM site_menu WHERE u_id='.$id.' AND l_id=1');
					...
					?>
						...
						<?PHP
							$s_sub = mysqli_query($connection,'SELECT * FROM site_menu WHERE l_id=1 and sub_id=0 and tip= '.$tip.' ORDER BY ordering ASC ');
				...
				<tr>
						...
						<div id="tabs">
								...
								<?PHP
									...
									while($nl = MYSQLI_FETCH_ARRAY($sl)){
									$dm=mysqli_query($connection,'SELECT * FROM site_menu WHERE u_id='.$id.' AND l_id='.$nl['id'].' limit 1');
