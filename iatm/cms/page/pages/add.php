<?PHP
	if(@$send){
		...
		for($i=1; $i<= $hidsay; $i++ ){
			...
			@$insert = MYSQLI_QUERY($connection,"INSERT INTO menu (sub_id, l_id, name, text, text2, status, tip, ordering, u_id, `url_tag`, title,`keyword`,`description`,`photo`, `link`) values('".$sub."', '".$i."', '".$name."', '".$text."', '".$text2."', '".$active."', '".$tip."', '".$getmax_order."','".$getmax."' , '$url_tag', '".$title."', '$keyword','$description','$avatar','$link')");
		}
		if($insert){
			...
			<table cellpadding="0" cellspacing="0" border="0" align="center">
				...
				<tr>
					...
					<select name="sub">
						...
						<?PHP 
                            $menu1 = mysqli_query($connection,'SELECT * FROM menu WHERE l_id=1 and tip= '.$tip.'');
