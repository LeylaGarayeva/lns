<?PHP
	...
	if(@$send){
		...
		for($i=1; $i<= $hidsay; $i++ ){
			...
			if(empty($avatar)){
			$update = MYSQLI_QUERY($connection,"UPDATE menu SET sub_id='$sub', name='$name', text='$text', text2='$text2', status='$active',  url_tag='$url_tag', title='$title',keyword='$keyword', description='$description', link='$link' WHERE u_id='$id' AND l_id='$i'");
			}
			else
			{
				$update = MYSQLI_QUERY($connection,"UPDATE menu SET sub_id='$sub', name='$name', text='$text', text2='$text2', status='$active', url_tag='$url_tag', title='$title',keyword='$keyword', description='$description', `photo` = '$avatar', link='$link' WHERE u_id='$id' AND l_id='$i'");
			}
		}
		if($update){
			...
			<table cellpadding="0" cellspacing="0" border="0" align="center">
				...
				<tr>
					...
					?>
						...
						<?PHP
							$s_sub = mysqli_query($connection,'SELECT * FROM menu WHERE l_id=1 and tip= '.$tip.' ORDER BY ordering ASC ');
