...
if(!empty($cat_id)){
	...
	if(isset($save)){
			...
			foreach($array as $k=>$vpppal) {
				foreach($vpppal as $ooo){
					$supdate = MYSQLI_QUERY($connection,"UPDATE kurs_date SET ordering='".$ooo."' WHERE u_id ='".$k."' ");
				}
			}
			if($supdate){
	...
	if(isset($delete)){
			...
			for($n=0; $n<count($chek); $n++)
			{
				...
				$s_del = MYSQLI_QUERY($connection,"DELETE FROM kurs_date WHERE u_id = '".$chek[$n]."'");
			}
			if($s_del){
	...
	if(isset($aktiv)){
			...
			for($n=0; $n<count($chek); $n++)
			{
				$supdate = MYSQLI_QUERY($connection,"UPDATE kurs_date SET s_id='0' WHERE u_id = '".$chek[$n]."'");
			}
			if($supdate){
	...
	if(isset($passiv)){
			...
			for($n=0; $n<count($chek); $n++)
			{
				$supdate = MYSQLI_QUERY($connection,"UPDATE kurs_date SET s_id='1' WHERE u_id = '".$chek[$n]."'");
			}
			if($supdate){
	...
	<table>
		...
		<tbody>
			<?PHP				
				...
				$s_say_menu = MYSQLI_QUERY($connection,"SELECT * FROM kurs_date where kurs_cat=".$cat_id." ORDER BY ordering DESC");
				...
				$s_m = MYSQLI_QUERY($connection,"SELECT * FROM kurs_date where kurs_cat=".$cat_id." ORDER BY ordering DESC limit ".$from.", ".$page_count."");
					...
					<tr>
						...
						<td>
							<!-- Icons -->
							 ...
							 <a href="?menu=kurs_date&cat_id=<?PHP echo $cat_id; ?>&mod=edit&id=<?PHP echo $n_m['u_id'];?>" title="<?php echo $dil['Düzəliş et'][$lng]; ?>">
								<img src="images/pencil.png" alt="Düzəliş et" />
							 </a>&nbsp;
							 <a href="?menu=kurs_date&cat_id=<?PHP echo $cat_id; ?>&mod=delete&id=<?PHP echo $n_m['u_id'];?>" title="<?php echo $dil['Sil'][$lng]; ?>">
			...
			<tr>
				<td colspan="9">
					<div class="pagination">
						...
						?>
						<a href="?menu=kurs_date&page=1" title="Birinci səhifə">&laquo; <?php echo $dil['Birinci'][$lng]; ?></a>
						<a href="?menu=kurs_date&page=<?PHP echo $page-1;?>" title="Geri"> &laquo; <?php echo $dil['Geri'][$lng]; ?> </a>
						<?PHP
						}	
							$paging = '';
							$kichikdir = 5;
							...
							$oterefbuteref=3;
								if($bolme_sayi <= $boyukdur){
									...
									$paging .= "<a href='?menu=kurs_date&page=".$nomre."' class='number  "; 
								...
								elseif($bolme_sayi > $kichikdir && $page <=$boyukdur && $page>=$kichikdir ){
									...
									$paging.= "....<a href='?menu=kurs_date&page=".$bolme_sayi."' class='number  "; 
								...
								elseif($bolme_sayi > $boyukdur && $page >=$boyukdur && $page <=($bolme_sayi-$kichikdir)  ){
									$paging.= "<a href='?menu=kurs_date&page=1' class='number  "; 
											if($bolme_sayi == $page) $paging.="current";
											$paging.="' title='1'>1</a>...";
						...
						?>
						<a href="?menu=kurs_date&page=<?PHP echo $page+1;?>" title="Irəli"> <?php echo $dil['İrəli'][$lng]; ?> &raquo;</a>
						<a href="?menu=kurs_date&page=<?PHP echo $bolme_sayi;?>" title="Sonuncu səhifə"> <?php echo $dil['Sonuncu'][$lng]; ?>&raquo;</a>
						<?PHP
						}
						?>
					</div> <!-- pagination -->
...
<?PHP } else{ ?>
	...
	<?PHP				
		...
		$s_m = MYSQLI_QUERY($connection,"SELECT * FROM kurs WHERE l_id='1'  ORDER BY ordering DESC");
		while($n_m = MYSQLI_FETCH_ASSOC($s_m)){
			?>
				<a style="display:block;padding:5px 0px;text-align:center;" href="?menu=kurs_date&cat_id=<?PHP echo $n_m['u_id'];?>"><?PHP echo $n_m['name'];?></a>
