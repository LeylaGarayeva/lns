<?PHP
	...
	if(isset($save)){
			...
			foreach($array as $k=>$vpppal) {
				foreach($vpppal as $ooo){
					$supdate = MYSQLI_QUERY($connection,"UPDATE xeberler SET ordering='".$ooo."' WHERE u_id ='".$k."' ");
				}
			}
			if($supdate){
	...
	if(isset($delete)){
			...
			{
				$product22=MYSQLI_QUERY($connection,"SELECT * from `xeberler` where l_id='1' and u_id='".$chek[$n]."'");
				$product=MYSQLI_FETCH_ARRAY($product22);
				unlink('../'.$product['photo'].'');
				$s_del = MYSQLI_QUERY($connection,"DELETE FROM xeberler WHERE u_id = '".$chek[$n]."'");
			}
			if($s_del){
	...
	if(isset($aktiv)){
			...
			for($n=0; $n<count($chek); $n++)
			{
				$supdate = MYSQLI_QUERY($connection,"UPDATE xeberler SET status='0' WHERE u_id = '".$chek[$n]."'");
			}
			if($supdate){
	...
	if(isset($passiv)){
			...
			for($n=0; $n<count($chek); $n++)
			{
				$supdate = MYSQLI_QUERY($connection,"UPDATE xeberler SET status='1' WHERE u_id = '".$chek[$n]."'");
			}
			if($supdate){
	...
	<table>
		...
		<tbody>
			<?PHP				
				...
				$s_say_menu = MYSQLI_QUERY($connection,"SELECT * FROM xeberler WHERE  l_id='1' AND tip='0' ORDER BY ordering DESC");
				...
				$s_m = MYSQLI_QUERY($connection,"SELECT * FROM xeberler WHERE  l_id='1' AND tip='0' ORDER BY ordering DESC limit ".$from.", ".$page_count."");
